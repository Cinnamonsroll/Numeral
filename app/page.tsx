  ...
  return (
    <>
      ...
      <div className="w-full h-screen bg-[var(--background)] text-[var(--foreground)] flex justify-center overflow-hidden">
        <div className="fixed top-4 left-4 z-50 hidden sm:flex">
          <a
            href="https://pancake.wtf"
            target="_blank"
            rel="noopener noreferrer"
            className="text-xs text-[var(--foreground)]/30 hover:text-[var(--theme)] transition-colors flex items-center gap-1"
          >
            {t("footer.madeWith")}{" "}
            <Image
        ...
        <div className="w-[25%] flex flex-col">
          <header className="w-full flex justify-center py-2">
            <div className="w-full h-10 px-3 rounded-lg bg-[var(--island)] flex items-center">
              <div className="flex sm:hidden w-full justify-between items-center">
                <button
                  onClick={() => setRulesOpen(true)}
                  className="text-[var(--foreground)]/60 hover:text-[var(--theme)] transition-colors"
                >
                  <svg
              ...
              <div className="hidden sm:flex w-full justify-between items-center">
                <div className="flex gap-2">
                  <button
                    onClick={() => setRulesOpen(true)}
                    className="hidden lg:block text-[var(--foreground)]/60 hover:text-[var(--theme)] transition-colors"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                ...
                <button
                  onClick={() => setStatsOpen(true)}
                  className="font-bold tracking-widest text-sm uppercase hover:text-[var(--theme)] transition-colors"
                >
                  {t("common.numeral")}
                </button>
                <div className="flex gap-2">
          ...
          <main className="flex-1 flex flex-col items-center justify-center gap-4 relative">
            ...
            <div className={`flex flex-col gap-1.5 ${shake ? "shake" : ""}`}>
              {rows.map((row, rowIndex) => (
                <div
                  key={rowIndex}
                  className={`flex gap-1.5 ${winRow === rowIndex ? "row-bounce" : ""}`}
                >
                  {row.map((cell, cellIndex) => {
                    ...
                    return (
                      <div
                        ...
                        style={
                          isRevealing
                            ? { animationDelay: `${cellIndex * 120}ms` }
                            : undefined
                        }
                      >
            ...
            {gameOver && !statsOpen && !winOpen && (
              <div
                className={`text-sm font-semibold px-5 py-2 rounded-full fade-up ${won ? "bg-[var(--theme)] text-white" : "bg-[var(--island)] border border-white/10 text-[var(--foreground)]/70"}`}
              >
                {won
                  ? `${t("game.nice")} 🎉`
                  : `${t("game.theNumberWas")} ${target}`}
              </div>
            )}
          </main>
            ...
            {keyboardRows.map((row, rowIndex) => (
              <div key={rowIndex} className="flex gap-1.5">
                {row.map((key) => {
                  ...
                  const colorClass = ks
                    ? ks === "correct"
                      ? "bg-[var(--correct)] border-[var(--correct)]"
                      : ks === "close"
                        ? "bg-[var(--close)] border-[var(--close)]"
                        : ks === "near"
                          ? "bg-[var(--near)] border-[var(--near)]"
                          : ks === "far"
                            ? "bg-[var(--far)] border-[var(--far)]"
                            : ks === "joker"
                              ? "bg-[var(--joker)] border-[var(--joker)]"
