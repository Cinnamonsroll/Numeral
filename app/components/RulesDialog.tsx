...
export function RulesDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { t } = useLang();
  const rules: { cls: string; tKey: string; num: string }[] = [
  ...
  const rules: { cls: string; tKey: string; num: string }[] = [
    { cls: "bg-[var(--correct)]", tKey: "exactMatch", num: "3" },
    { cls: "bg-[var(--close)]",   tKey: "close", num: "5" },
    { cls: "bg-[var(--near)]",    tKey: "near", num: "2" },
    { cls: "bg-[var(--far)]",     tKey: "far", num: "8" },
    { cls: "bg-[var(--joker)]",   tKey: "joker", num: "∞" },
  ];
